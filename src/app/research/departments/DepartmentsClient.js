...
export default function DepartmentsClient() {
  ...
  const globalProjects = useMemo(() => {
    ...
    for (const person of allStaff) {
      const projs = (person.projects ?? []).map((proj) =>
        typeof proj === "string"
          ? {
              title: proj,
              lead: undefined,
  ...
  const globalPublications = useMemo(() => {
    const rows = [];
    for (const person of allStaff) {
      const pubs = (person.publications ?? []).map((pub) =>
        typeof pub === "string"
  ...
  const unitProjects = useMemo(() => {
    ...
    return globalProjects.filter((p) => p.domain === selectedUnit.name);
  }, [selectedUnit, globalProjects]);

  const unitPublications = useMemo(() => {
    if (!selectedUnit) return [];
    return globalPublications.filter((p) => p.domain === selectedUnit.name);
  ...
  const unitThemes = useMemo(() => {
    if (!selectedUnit) return [];
    return globalThemes.filter((t) => t.domain === selectedUnit.name);
